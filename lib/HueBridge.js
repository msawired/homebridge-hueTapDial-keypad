...
class HueBridge {
  ...
  async exposeBridge () {
    ...
    // Rely solely on legacy users map in config
    this.apiKey = (this.platform.config.users && this.platform.config.users[this.serialNumber]) || ''
    this.config = {
      parallelRequests: 10,
    ...
    // Intentionally do not create any HomeKit services for the bridge itself.
    // This keeps internal HTTP/event setup intact without exposing a bridge accessory.
  }
  ...
  async createUser () {
    ...
    try {
      this.apiKey = await this.hueClient.getApiKey('homebridge-hue-dial-keypad')
      await this._persistUserApiKey()
      let s = '\n'
      s += '  "platforms": [\n'
      s += '    {\n'
      s += '      "platform": "HueDialKeypad",\n'
      s += '      "users": {\n'
  ...
  async _persistUserApiKey () {
    try {
      // Update in-memory config first
      this.platform.config.userId = this.serialNumber
      this.platform.config.userToken = this.apiKey
      this.platform.config.users = this.platform.config.users || {}
      this.platform.config.users[this.serialNumber] = this.apiKey

      // Try to persist into Homebridge config.json so it's backed up by HB UI
      const configPath = this.platform.api.user.storagePath() + '/config.json'
      try {
        ...
        if (cfg && Array.isArray(cfg.platforms)) {
          ...
          for (const p of cfg.platforms) {
            if (p && p.platform === 'HueDialKeypad') {
              p.userId = this.serialNumber
              p.userToken = this.apiKey
              p.users = p.users || {}
              p.users[this.serialNumber] = this.apiKey
              modified = true
            }
          }
          if (modified) {
            await writeFile(configPath, JSON.stringify(cfg, null, 2))
            this.log.info('%s: stored API key in Homebridge config.json', this.name)
            return
          }
        }
      } catch (e) {
        // Could not read/modify config.json; fall back to logging snippet
        this.log.debug('%s: cannot update config.json automatically: %s', this.name, e.message || e)
      }
      // Fallback: log snippet for manual copy into UI
      const snippet = {
        userId: this.serialNumber,
        userToken: this.apiKey
      }
      this.log.warn('%s: please update the HueDialKeypad platform config with: %s', this.name, JSON.stringify(snippet))
    } catch (e) {
      this.log.warn('%s: cannot persist API key: %s', this.name, e.message || e)
    }
  }
  ...
  exposeSensors (sensors) {
    for (const id in sensors) {
      ...
      // Only expose Philips Hue Tap Dial components (modelid RDM002)
      if (sensor.modelid !== 'RDM002') {
        this.log.debug('%s: /sensors/%d: %s (%s) excluded: not RDM002', this.name, id, sensor.type, sensor.modelid)
        continue
      }
  ...
  async heartbeatConfig (beat) {
    ...
    if (config.linkbutton !== this.state.linkbutton) {
      ...
      if (this.state.linkbutton) {
        this.log(
          '%s: homekit linkbutton single press', this.switchService ? this.switchService.displayName : this.name
        )
