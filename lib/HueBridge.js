...
class HueBridge {
  ...
  async exposeBridge () {
    ...
    this.serialNumber = String(this.bridge.bridgeid).toUpperCase()
    this.uuid_base = this.serialNumber
    const users = this.platform.config.users || {}
    this.apiKey = users[this.serialNumber] || ''
    this.config = {
      parallelRequests: 10,
  ...
  async createUser () {
    ...
    try {
      ...
      try {
        ...
        // Fallback: instruct manual edit
        let s = '\n'
        s += '  "platforms": [\n'
        s += '    {\n'
        s += '      "platform": "HueDialKeypad",\n'
        s += '      "userId": "' + this.serialNumber + '",\n'
        s += '      "userToken": "' + this.apiKey + '",\n'
        s += '      "users": {\n'
        ...
        this.log.debug('%s: cannot update config.json automatically: %s', this.name, persistErr && persistErr.message ? persistErr.message : persistErr)
      }
    } catch (error) {
  ...
  async _persistUserApiKey () {
    // Update in-memory platform config
    const id = String(this.serialNumber).toUpperCase()
    this.platform.config.userId = id
    this.platform.config.userToken = this.apiKey
    this.platform.config.users = this.platform.config.users || {}
    this.platform.config.users[id] = this.apiKey

    // Update Homebridge config.json on disk
