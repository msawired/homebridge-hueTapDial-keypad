...
class HueBridge {
  ...
  async exposeBridge () {
    ...
    // Do not expose the Hue bridge itself as an accessory in HomeKit.
  ...
  async _persistUserApiKey () {
    try {
      const cacheFile = this.platform.api.user.storagePath() + '/' + this.platform.packageJson.name + '-users.json'
      let cached = { users: {} }
      try {
        const buf = await readFile(cacheFile)
        const json = JSON.parse(buf.toString())
        if (json && typeof json === 'object' && json.users && typeof json.users === 'object') {
          cached = json
        }
      } catch (e) {
        // No cache yet; create new.
      }
      cached.users[this.serialNumber] = this.apiKey
      await writeFile(cacheFile, JSON.stringify(cached, null, 2))
      this.log.info('%s: stored API key in %s', this.name, cacheFile)
    } catch (e) {
