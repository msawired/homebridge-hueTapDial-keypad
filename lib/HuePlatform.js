...
class HuePlatform {
  constructor (log, configJson, homebridge) {
    ...
    const apiKeys = []
    if (this.configJson && this.configJson.users) {
      for (const bridgeId in this.configJson.users) {
        apiKeys.push(this.configJson.users[bridgeId])
      }
    }
    if (this.configJson && this.configJson.userToken) {
      apiKeys.push(this.configJson.userToken)
    }
    this.log.debug(
      'config.json: %s',
    ...
    try {
      ...
      // Normalize existing users map keys to uppercase bridge IDs
      if (this.config.users) {
        const normalized = {}
        for (const k of Object.keys(this.config.users)) {
          normalized[String(k).toUpperCase()] = this.config.users[k]
        }
        this.config.users = normalized
      }
      // Reconstruct users map from convenience fields if provided
      if (this.config.userId && this.config.userToken) {
        const id = String(this.config.userId).toUpperCase()
        this.config.users = this.config.users || {}
        this.config.users[id] = this.config.userToken
      }
      if (this.config.hosts.length > 0) {
        for (const i in this.config.hosts) {
          try {
            const { port } = OptionParser.toHost(`config.json: hosts[${i}]`, this.config.hosts[i], false, true)
            if (port != null) {
              this.log.warn(`config.json: hosts[${i}]: ${this.config.hosts[i]}: ignoring port`)
            }
          } catch (error) {
            this.log.warn('config.json: %s', formatError(error))
          }
      ...
      // Removed legacy users cache merge: credentials now live in Homebridge config.
    } catch (error) {
    ...
    this.log.debug(
      'config: %s',
      this.maskApiKeys(this.config.userToken ? apiKeys.concat(this.config.userToken) : apiKeys, this.maskConfigJson(this.config))
    )
    ...
    this.hueDiscovery
      ...
      .on('searching', (name, host) => {
        this.log.debug('%s: listening on %s', name, host)
      })
      .on('searchDone', (name) => { this.log.debug('%s: search done', name) })
