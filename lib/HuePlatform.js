...
class HuePlatform {
  constructor (log, configJson, homebridge) {
    ...
    try {
      ...
      // Merge cached users (persisted API keys) if present.
      try {
        const cacheFile = this.api.user.storagePath() + '/' + packageJson.name + '-users.json'
        if (existsSync(cacheFile)) {
          const buf = readFileSync(cacheFile)
          const cached = JSON.parse(buf.toString())
          if (cached && typeof cached === 'object' && cached.users && typeof cached.users === 'object') {
            for (const bridgeId of Object.keys(cached.users)) {
              if (!this.config.users[bridgeId]) {
                this.config.users[bridgeId] = cached.users[bridgeId]
              }
            }
          }
        }
      } catch (e) {
        // No cache yet or unreadable; ignore.
      }
    } catch (error) {
