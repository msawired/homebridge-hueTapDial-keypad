...
class HueSensor {
          ...
          case 'Signify Netherlands B.V.': {
            ...
            switch (this.obj.modelid) {
              ...
              case 'RDM002': // Hue tap dial switch
                ...
                // Do NOT create button services for HomeKit; we expose only the keypad success sensor.
                // Initialise keypad state for password mode
                this.keypad = {
                  enabled: true,
                  password: (this.bridge.platform.config.password || '1234').trim(),
                  inCode: false,
                  buffer: '',
                ...
                // Success signal service (MotionSensor)
                this.keypadService = new eve.Services.MotionSensor(this.name + ' Keypad', 'keypad')
                this.serviceList.push(this.keypadService)
                break
  ...
  // ===== Keypad (RDM002) helpers =============================================

  _startKeypadTimeout () {
    // 3s inactivity -> exit code mode and emit original first press
    if (this.keypad?.timeout != null) {
      clearTimeout(this.keypad.timeout)
    }
    this.keypad.timeout = setTimeout(() => {
      const ki = this.keypad.firstButtonIndex
      this.log.debug('%s: keypad timeout - exiting code mode%s', this.name,
        ki != null ? ' and emitting first press' : '')
      this.keypad.inCode = false
      this.keypad.buffer = ''
      // Emit first press as normal single press if captured
      if (ki != null && this.buttonMap[ki] != null) {
        const char = this.buttonMap[ki]
          .getCharacteristic(Characteristic.ProgrammableSwitchEvent)
        if (char.props.validValues.includes(Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS)) {
          this.log.info('%s: homekit button %s single press (fallback)',
            this.buttonMap[ki].displayName, '"' + ki + '"')
          char.updateValue(Characteristic.ProgrammableSwitchEvent.SINGLE_PRESS)
        }
      }
      this.keypad.firstButtonIndex = null
    }, 3000)
  }

  _triggerKeypadSuccess () {
    if (!this.keypadService) {
      return
    }
    // Pulse motion true for 5s
    this.keypadService.getCharacteristic(Characteristic.MotionDetected)
      .updateValue(1)
    if (this.keypad.successTimer != null) {
      clearTimeout(this.keypad.successTimer)
    }
    this.keypad.successTimer = setTimeout(() => {
      this.keypadService.getCharacteristic(Characteristic.MotionDetected)
        .updateValue(0)
    }, 5000)
  }
  ...
  checkButtonevent (rawEvent, lastupdated) {
    ...
    if (event !== previousEvent || lastupdated > this.obj.state.lastupdated) {
      ...
      // Intercept keypad digits for RDM002 ZLLSwitch
      if (
        this.obj.type === 'ZLLSwitch' &&
        this.obj.modelid === 'RDM002' &&
        this.keypad?.enabled &&
        buttonIndex != null && [1, 2, 3, 4].includes(buttonIndex)
      ) {
        const evt = event % 1000
        if (evt === SHORT_RELEASE) {
          const digit = String(buttonIndex)
          const pwd = this.keypad.password
          if (!this.keypad.inCode) {
            // Start code mode only if first digit matches
            if (digit === pwd.charAt(0)) {
              this.keypad.inCode = true
              this.keypad.buffer = digit
              this.keypad.firstButtonIndex = buttonIndex
              this.log.debug('%s: keypad: start code entry (%s*)', this.name, digit)
              this._startKeypadTimeout()
              return // suppress normal event
            }
            // Not entering code mode -> pass through
          } else {
            // In code mode: accumulate
            this.keypad.buffer += digit
            this.log.debug('%s: keypad buffer: %s', this.name, this.keypad.buffer)
            this._startKeypadTimeout()
            if (this.keypad.buffer.length >= pwd.length) {
              const ok = this.keypad.buffer === pwd
              this.keypad.inCode = false
              this.keypad.buffer = ''
              const first = this.keypad.firstButtonIndex
              this.keypad.firstButtonIndex = null
              if (ok) {
                this.log.info('%s: keypad: password accepted', this.name)
                this._triggerKeypadSuccess()
                return // suppress all normal events
              }
              this.log.info('%s: keypad: password rejected', this.name)
              // Wrong code: suppress events
              return
            }
            return // keep suppressing normal events while entering
          }
        }
        // For other press types while in code mode: suppress
        if (this.keypad?.inCode) {
          return
        }
      }
